    ...
    if( ((NPC==0) || (NPC==2)) && (NVE==1 || NVE==3))
    {
	 ...
	 for (p=0; p<ngrids; p++){
	    ...
	    //velocity computation in y boundary, two ghost cell regions
            ...
            Cpy2Host_VY(d_f_u1[p], d_f_v1[p], d_f_w1[p], SF_vel[p], nxt[p],
                        nzt[p], stream_1, y_rank_F);
            if (!usetopo || p > 0) {
                    dvelcy_H(d_u1[p], d_v1[p], d_w1[p], d_xx[p], d_yy[p],
                             d_zz[p], d_xy[p], d_xz[p], d_yz[p], d_dcrjx[p],
                             d_dcrjy[p], d_dcrjz[p], d_d1[p], nxt[p], nzt[p],
                             d_b_u1[p], d_b_v1[p], d_b_w1[p], stream_2, ybs[p],
                             ybe[p], y_rank_B, p);
            } else {
#if TOPO
            ...
            if (!usetopo || p > 0) {
                    dvelcx_H_opt(d_u1[p], d_v1[p], d_w1[p], d_xx[p], d_yy[p],
                                 d_zz[p], d_xy[p], d_xz[p], d_yz[p], d_dcrjx[p],
                                 d_dcrjy[p], d_dcrjz[p], d_d1[p], nyt[p],
                                 nzt[p], stream_i, xvs[p], xve[p], p, ngrids);
            } else {
#if TOPO
            ...
            for (p = 0; p < ngrids; p++) {
                    dump_nonzeros(d_u1[p], nxt[p] + 4 + 8 * loop,
                                  ...
                                  "u1", p, cur_step, 7, rank, size);
                    dump_nonzeros(d_v1[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "v1", p, cur_step, 7, rank, size);
                    dump_nonzeros(d_w1[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "w1", p, cur_step, 7, rank, size);
            }
            ...
            CUCHK(cudaStreamSynchronize(stream_1));
            for (p = 0; p < ngrids; p++) {
                    PostSendMsg_Y(SF_vel[p], SB_vel[p], MCW, request_y[p],
                                  &count_y[p], msg_v_size_y[p], y_rank_F,
                                  y_rank_B, rank, Front, p);
            }
            CUCHK(cudaStreamSynchronize(stream_2));
            for (p = 0; p < ngrids; p++) {
                    PostSendMsg_Y(SF_vel[p], SB_vel[p], MCW, request_y[p],
                                  &count_y[p], msg_v_size_y[p], y_rank_F,
                                  y_rank_B, rank, Back, p);
                    MPICHK(MPI_Waitall(count_y[p], request_y[p], status_y[p]));
                    Cpy2Device_VY(d_u1[p], d_v1[p], d_w1[p], d_f_u1[p],
                                  d_f_v1[p], d_f_w1[p], d_b_u1[p], d_b_v1[p],
                                  d_b_w1[p], RF_vel[p], RB_vel[p], nxt[p],
                                  nyt[p], nzt[p], stream_1, stream_2, y_rank_F,
                                  y_rank_B, p);
            }
            ...
            for (p = 0; p < ngrids - 1; p++) {
                    dvelc2_H(d_u1[p], d_v1[p], d_w1[p], d_xx[p], d_yy[p],
                             d_zz[p], d_xy[p], d_xz[p], d_yz[p], d_dcrjx[p],
                             d_dcrjy[p], d_dcrjz[p], d_d1[p], nxt[p], nyt[p],
                             stream_i, p);
            }
            for (p = 0; p < ngrids; p++) {
                    dump_nonzeros(d_u1[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "u1", p, cur_step, 0, rank, size);
                    dump_nonzeros(d_v1[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "v1", p, cur_step, 0, rank, size);
                    dump_nonzeros(d_w1[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "w1", p, cur_step, 0, rank, size);
            }
            ...
            for (p = 0; p < ngrids; p++) {
                    dump_all_data(d_u1[p], d_v1[p], d_w1[p], d_xx[p], d_yy[p],
                                  d_zz[p], d_xz[p], d_yz[p], d_xy[p], nel[p],
                                  cur_step, 0, p, rank, size);
            }

            if ((rank == srcproc[0]) && (IFAULT == 4)) {
                    fprintf(stdout, "calling frcvel_H\n");
                    ++source_step;
                    frcvel_H(source_step, READ_STEP_GPU, maxdim, d_tpsrc[0],
                             npsrc[0], fbc_tskp, stream_i, d_taxx[0], d_tayy[0],
                             d_tazz[0], d_taxz[0], d_tayz[0], d_taxy[0],
                             d_u1[0], d_v1[0], d_w1[0], -1, -1, 0);
            }
            CUCHK(cudaStreamSynchronize(stream_i));
            for (p = 0; p < ngrids; p++) {
                    dump_nonzeros(d_xx[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xx", p, cur_step, 2, rank, size);
                    dump_nonzeros(d_yy[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "yy", p, cur_step, 2, rank, size);
                    dump_nonzeros(d_zz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "zz", p, cur_step, 2, rank, size);
                    dump_nonzeros(d_xy[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xy", p, cur_step, 2, rank, size);
                    dump_nonzeros(d_xz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xz", p, cur_step, 2, rank, size);
                    dump_nonzeros(d_yz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "yz", p, cur_step, 2, rank, size);
            }

            for (p = 0; p < ngrids; p++) {
                    PostRecvMsg_X(RL_vel[p], RR_vel[p], MCW, request_x[p],
                                  &count_x[p], msg_v_size_x[p], x_rank_L,
                                  x_rank_R, p);
            }
            if (NVE < 3) {
                    ...
                    for (p = 0; p < ngrids; p++) {
                            ...
                            if (!usetopo || p > 0) {
                                    dstrqc_H(
                                        ...
                                        d_xz[p], d_yz[p], d_r1[p], d_r2[p],
                                        d_r3[p], d_r4[p], d_r5[p], d_r6[p],
                                        d_u1[p], d_v1[p], d_w1[p], d_lam[p],
                                        d_mu[p], d_qp[p], d_coeff, d_qs[p],
                                        d_dcrjx[p], d_dcrjy[p], d_dcrjz[p],
                                        nyt[p], nzt[p], stream_i, d_lam_mu[p],
                                        d_vx1[p], d_vx2[p], d_ww[p], d_wwo[p],
                                        NX * grdfct[p], NPC, coord[0], coord[1],
                                        xss2[p], xse2[p], yls[p], yre[p], p);
                            } else {
#if TOPO
                    ...
                    for (p = 0; p < ngrids; p++) {
                            dstrqc_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p],
                                     d_xz[p], d_yz[p], d_r1[p], d_r2[p],
                                     d_r3[p], d_r4[p], d_r5[p], d_r6[p],
                                     d_u1[p], d_v1[p], d_w1[p], d_lam[p],
                                     d_mu[p], d_qp[p], d_coeff, d_qs[p],
                                     d_dcrjx[p], d_dcrjy[p], d_dcrjz[p], nyt[p],
                                     nzt[p], stream_i, d_lam_mu[p], d_vx1[p],
                                     d_vx2[p], d_ww[p], d_wwo[p],
                                     NX * grdfct[p], NPC, coord[0], coord[1],
                                     xss2[p], xse2[p], yls[p], yls2[p] - 1, p);
                            dstrqc_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p],
                                     d_xz[p], d_yz[p], d_r1[p], d_r2[p],
                                     d_r3[p], d_r4[p], d_r5[p], d_r6[p],
                                     d_u1[p], d_v1[p], d_w1[p], d_lam[p],
                                     d_mu[p], d_qp[p], d_coeff, d_qs[p],
                                     d_dcrjx[p], d_dcrjy[p], d_dcrjz[p], nyt[p],
                                     nzt[p], stream_i2, d_lam_mu[p], d_vx1[p],
                                     d_vx2[p], d_ww[p], d_wwo[p],
                                     NX * grdfct[p], NPC, coord[0], coord[1],
                                     xss2[p], xse2[p], yre2[p] + 1, yre[p], p);
                    }
            }

            // dump_all_stresses(d_xx, d_yy, d_zz, d_xz, d_yz, d_xy, nel, 'u',
            // cur_step, 0, rank, size);

            for (p = 0; p < ngrids; p++) {
                    Cpy2Host_VX(d_u1[p], d_v1[p], d_w1[p], SL_vel[p], nxt[p],
                                nyt[p], nzt[p], stream_1, x_rank_L, Left);
                    Cpy2Host_VX(d_u1[p], d_v1[p], d_w1[p], SR_vel[p], nxt[p],
                                nyt[p], nzt[p], stream_2, x_rank_R, Right);
            }

            // velocity communication in x direction
            CUCHK(cudaStreamSynchronize(stream_1));
            for (p = 0; p < ngrids; p++) {
                    PostSendMsg_X(SL_vel[p], SR_vel[p], MCW, request_x[p],
                                  &count_x[p], msg_v_size_x[p], x_rank_L,
                                  x_rank_R, rank, Left, p);
            }
            CUCHK(cudaStreamSynchronize(stream_2));
            for (p = 0; p < ngrids; p++) {
                    PostSendMsg_X(SL_vel[p], SR_vel[p], MCW, request_x[p],
                                  &count_x[p], msg_v_size_x[p], x_rank_L,
                                  x_rank_R, rank, Right, p);
                    MPICHK(MPI_Waitall(count_x[p], request_x[p], status_x[p]));

                    Cpy2Device_VX(d_u1[p], d_v1[p], d_w1[p], RL_vel[p],
                                  RR_vel[p], nxt[p], nyt[p], nzt[p], stream_1,
                                  stream_2, x_rank_L, x_rank_R);
                    ...
                    if (!usetopo || p > 0) {
                            dstrqc_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p],
                                     ...
                                     d_mu[p], d_qp[p], d_coeff, d_qs[p],
                                     d_dcrjx[p], d_dcrjy[p], d_dcrjz[p], nyt[p],
                                     nzt[p], stream_1, d_lam_mu[p], d_vx1[p],
                                     d_vx2[p], d_ww[p], d_wwo[p],
                                     NX * grdfct[p], NPC, coord[0], coord[1],
                                     xss1[p], xse1[p], yls[p], yre[p], p);
                            dstrqc_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p],
                                     d_xz[p], d_yz[p], d_r1[p], d_r2[p],
                                     d_r3[p], d_r4[p], d_r5[p], d_r6[p],
                                     d_u1[p], d_v1[p], d_w1[p], d_lam[p],
                                     d_mu[p], d_qp[p], d_coeff, d_qs[p],
                                     d_dcrjx[p], d_dcrjy[p], d_dcrjz[p], nyt[p],
                                     nzt[p], stream_2, d_lam_mu[p], d_vx1[p],
                                     d_vx2[p], d_ww[p], d_wwo[p],
                                     NX * grdfct[p], NPC, coord[0], coord[1],
                                     xss3[p], xse3[p], yls[p], yre[p], p);
                    } else {
#if TOPO
            ...
            for (p = 0; p < ngrids; p++) {
                    dump_nonzeros(d_xx[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xx", p, cur_step, 8, rank, size);
                    dump_nonzeros(d_yy[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "yy", p, cur_step, 8, rank, size);
                    dump_nonzeros(d_zz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "zz", p, cur_step, 8, rank, size);
                    dump_nonzeros(d_xy[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xy", p, cur_step, 8, rank, size);
                    dump_nonzeros(d_xz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xz", p, cur_step, 8, rank, size);
                    dump_nonzeros(d_yz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "yz", p, cur_step, 8, rank, size);
            }

            for (p = 0; p < ngrids - 1; p++) {
                    dstrqc2_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p], d_xz[p],
                              d_yz[p], d_r1[p], d_r2[p], d_r3[p], d_r4[p],
                              d_r5[p], d_r6[p], d_u1[p], d_v1[p], d_w1[p],
                              d_lam[p], d_mu[p], d_qp[p], d_qs[p], d_dcrjx[p],
                              d_dcrjy[p], d_dcrjz[p], nxt[p], nyt[p], stream_i,
                              d_coeff, d_vx1[p], d_vx2[p], d_ww[p], d_wwo[p],
                              xss1[p], xse3[p], yls[p], yre[p], p);
                    /*print_nan_H(d_xx[p], (nxt[p]+4+ngsl2), (nyt[p]+4+ngsl2),
                    (nzt[p]+2*align), "xx"); print_nan_H(d_yy[p],
                    (nxt[p]+4+ngsl2), (nyt[p]+4+ngsl2), (nzt[p]+2*align), "yy");
                    print_nan_H(d_zz[p], (nxt[p]+4+ngsl2), (nyt[p]+4+ngsl2),
                    (nzt[p]+2*align), "zz"); print_nan_H(d_xy[p],
                    (nxt[p]+4+ngsl2), (nyt[p]+4+ngsl2), (nzt[p]+2*align), "xy");
                    print_nan_H(d_xz[p], (nxt[p]+4+ngsl2), (nyt[p]+4+ngsl2),
                    (nzt[p]+2*align), "xz"); print_nan_H(d_yz[p],
                    (nxt[p]+4+ngsl2), (nyt[p]+4+ngsl2), (nzt[p]+2*align),
                    "yz");*/
            }
            ...
            for (p = 0; p < ngrids; p++) {
                    dump_nonzeros(d_xz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "xz", p, cur_step, 4, rank, size);
                    dump_nonzeros(d_yz[p], nxt[p] + 4 + 8 * loop,
                                  nyt[p] + 4 + 8 * loop, nzt[p] + 2 * align,
                                  "yz", p, cur_step, 4, rank, size);
            }
            ...
            if (NVE == 3) {
                    ...
                    for (p = 0; p < ngrids; p++) {
                            PostRecvMsg_Y(
                                RF_yldfac[p], RB_yldfac[p], MCW,
                                request_y_yldfac[p], &count_y_yldfac[p],
                                yldfac_msg_size_y[p], y_rank_F, y_rank_B, p);
                            PostRecvMsg_X(
                                RL_yldfac[p], RR_yldfac[p], MCW,
                                request_x_yldfac[p], &count_x_yldfac[p],
                                yldfac_msg_size_x[p], x_rank_L, x_rank_R, p);

                            // yield factor computation, front and back
                            drprecpc_calc_H_opt(
                                d_xx[p], d_yy[p], d_zz[p], d_xy[p], d_xz[p],
                                d_yz[p], d_mu[p], d_d1[p], d_sigma2[p],
                                d_yldfac[p], d_cohes[p], d_phi[p], d_neta[p],
                                nzt[p], xlsp[p], xrep[p], ylsp[p],
                                ylsp[p] + ngsl, stream_1, p);
                            drprecpc_calc_H_opt(
                                d_xx[p], d_yy[p], d_zz[p], d_xy[p], d_xz[p],
                                d_yz[p], d_mu[p], d_d1[p], d_sigma2[p],
                                d_yldfac[p], d_cohes[p], d_phi[p], d_neta[p],
                                nzt[p], xlsp[p], xrep[p], yrep[p] - ngsl,
                                yrep[p], stream_2, p);
                            update_yldfac_buffer_y_H(
                                d_yldfac[p], d_SF_yldfac[p], d_SB_yldfac[p],
                                nxt[p], nzt[p], stream_1, stream_2, y_rank_F,
                                y_rank_B, p);
                    }
                    CUCHK(cudaStreamSynchronize(stream_1));
                    CUCHK(cudaStreamSynchronize(stream_2));
                    ...
                    for (p = 0; p < ngrids; p++) {
                            Cpy2Host_yldfac_Y(
                                d_yldfac[p], SF_yldfac[p], SB_yldfac[p],
                                d_SF_yldfac[p], d_SB_yldfac[p], nxt[p], nzt[p],
                    ...
                    for (p = 0; p < ngrids; p++) {
                            dstrqc_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p],
                                     ...
                                     d_vx2[p], d_ww[p], d_wwo[p],
                                     NX * grdfct[p], NPC, coord[0], coord[1],
                                     xss2[p], xse2[p], yls2[p], yre2[p], p);
                    }

                    CUCHK(cudaStreamSynchronize(stream_1));
                    CUCHK(cudaStreamSynchronize(stream_2));
                    for (p = 0; p < ngrids; p++) {
                            PostSendMsg_Y(SF_yldfac[p], SB_yldfac[p], MCW,
                                          request_y_yldfac[p],
                                          &count_y_yldfac[p],
                                          yldfac_msg_size_y[p], y_rank_F,
                                          y_rank_B, rank, Both, p);
                            MPICHK(MPI_Waitall(count_y_yldfac[p],
                                               request_y_yldfac[p],
                                               status_y_yldfac[p]));
                            ...
                            drprecpc_calc_H_opt(
                                ...
                                d_yz[p], d_mu[p], d_d1[p], d_sigma2[p],
                                d_yldfac[p], d_cohes[p], d_phi[p], d_neta[p],
                                nzt[p], xlsp[p], xlsp[p] + ngsl, ylsp[p],
                                yrep[p], stream_1, p);
                            drprecpc_calc_H_opt(
                                d_xx[p], d_yy[p], d_zz[p], d_xy[p], d_xz[p],
                                d_yz[p], d_mu[p], d_d1[p], d_sigma2[p],
                                d_yldfac[p], d_cohes[p], d_phi[p], d_neta[p],
                                nzt[p], xrep[p] - ngsl, xrep[p], ylsp[p],
                                yrep[p], stream_2, p);

                            Cpy2Device_yldfac_Y(
                                d_yldfac[p], RF_yldfac[p], RB_yldfac[p],
                                d_RF_yldfac[p], d_RB_yldfac[p], nxt[p], nzt[p],
                                stream_1, stream_2, y_rank_F, y_rank_B, p);
                            update_yldfac_buffer_x_H(
                                d_yldfac[p], d_SL_yldfac[p], d_SR_yldfac[p],
                                nyt[p], nzt[p], stream_1, stream_2, x_rank_L,
                                x_rank_R, p);
                    }
                    ...
                    for (p = 0; p < ngrids; p++) {
                            Cpy2Host_yldfac_X(
                                d_yldfac[p], SL_yldfac[p], SR_yldfac[p],
                                d_SL_yldfac[p], d_SR_yldfac[p], nyt[p], nzt[p],
                            ...
                            drprecpc_calc_H_opt(
                                ...
                                d_yz[p], d_mu[p], d_d1[p], d_sigma2[p],
                                d_yldfac[p], d_cohes[p], d_phi[p], d_neta[p],
                                nzt[p], xlsp[p] + ngsl, xrep[p] - ngsl,
                                ylsp[p] + ngsl, yrep[p] - ngsl, stream_i,
                                p); /*xrep-ngsl*/
                    }

                    // dump_variable(d_yldfac, nel, "yldfac", 'u', cur_step, 0,
                    // rank, size);

                    // dump_variable(d_yldfac, nel, "yldfac", 'u', cur_step, 1,
                    // rank, size);
                    ...
                    for (p = 0; p < ngrids; p++) {
                            PostSendMsg_X(SL_yldfac[p], SR_yldfac[p], MCW,
                                          request_x_yldfac[p],
                                          &count_x_yldfac[p],
                                          yldfac_msg_size_x[p], x_rank_L,
                                          x_rank_R, rank, Both, p);
                            MPICHK(MPI_Waitall(count_x_yldfac[p],
                                               request_x_yldfac[p],
                                               status_x_yldfac[p]));
                            Cpy2Device_yldfac_X(
                                d_yldfac[p], RL_yldfac[p], RR_yldfac[p],
                                d_RL_yldfac[p], d_RR_yldfac[p], nyt[p], nzt[p],
                                stream_1, stream_2, x_rank_L, x_rank_R, p);
                    }

                    // wait until all streams have completed, including stream_i
                    // working on the inside part
                    CUCHK(cudaDeviceSynchronize());
                    // dump_variable(d_yldfac, nel, "yldfac", 'u', cur_step, 2,
                    // rank, size);

                    for (p = 0; p < ngrids; p++) {
                            drprecpc_app_H(d_xx[p], d_yy[p], d_zz[p], d_xy[p],
                                           d_xz[p], d_yz[p], d_mu[p],
                                           d_sigma2[p], d_yldfac[p], nzt[p],
                                           xlsp[p], xrep[p], ylsp[p], yrep[p],
                                           stream_i, p);
                    }
            }
            ...
            /*if ((IFAULT < 4) && (cur_step<NST)) {
               ...
               for (p=0; p<ngrids; p++){
                  if (rank==srcproc[p])
                  addsrc_H(source_step, READ_STEP_GPU, maxdim, d_tpsrc[p],
            npsrc[p], stream_i, d_taxx[p], d_tayy[p], d_tazz[p], d_taxz[p],
            d_tayz[p], d_taxy[p], d_xx[p], d_yy[p], d_zz[p], d_xy[p], d_yz[p],
            d_xz[p], p);
               }
            }
            if (IFAULT == 5) {
               ...
               for (p=0; p<ngrids; p++){
                  if (rank==srcproc[p])
                  addkinsrc_H(cur_step, maxdim, d_tpsrc[p], npsrc[p], stream_i,
            d_mu[p], d_taxx[p], d_tayy[p], d_tazz[p], d_taxz[p], d_tayz[p],
            d_taxy[p], d_xx[p], d_yy[p], d_zz[p], d_xy[p], d_yz[p], d_xz[p],
            d_mom[p], p);
               }
            }*/
            ...
            if (cur_step < -1) {
                    for (p = 0; p < ngrids; p++)
                            // print_nonzero_H(d_xx[p], (nxt[p]+4+ngsl2),
                            // (nyt[p]+4+ngsl2), (nzt[p]+2*align), p);
                            print_nonzero_mat_H(d_xx[p], (nxt[p] + 4 + ngsl2),
                                                (nyt[p] + 4 + ngsl2),
                                                (nzt[p] + 2 * align), p,
                                                d_d1[p], d_mu[p], d_lam[p],
                                                d_qp[p], d_qs[p], rank);
            }
...
// cudaDeviceSynchronize();
                    ...
                    for (p = 0; p < ngrids; p++) {
                        ...
                        // This should save the final plastic strain tensor at
                        // the end of the simulation
                        if (NVE == 3) {
#ifndef SEISMIO
                                Bufeta =
                                    (Grid1D*)calloc(ngrids, sizeof(Grid1D));
                                for (p = 0; p < ngrids; p++) {
                                        fprintf(stdout,
                                                "copying plastic strain back "
                                                "to CPU\n");
                                        num_bytes = sizeof(float) *
                                                    ...
                                                    (nzt[p] + 2 * align);
                                        CUCHK(cudaMemcpy(
                                            &neta[p][0][0][0], d_neta[p],
                                            num_bytes, cudaMemcpyDeviceToHost));
                                        tmpInd = 0;

                                        rec_NZ[p] =
                                            (NEDZ_EP - NBGZ[p]) / NSKPZ[p] + 1;
                                        calcRecordingPoints(
                                            &rec_nbgx[p], &rec_nedx[p],
                                            &rec_nbgy[p], &rec_nedy[p],
                                            &rec_nbgz[p], &rec_nedz[p],
                                            &rec_nxt[p], &rec_nyt[p],
                                            &rec_nzt[p], &displacement[p],
                                            (long int)nxt[p], (long int)nyt[p],
                                            (long int)nzt[p], rec_NX[p],
                                            rec_NY[p], rec_NZ[p], NBGX[p],
                                            NEDX[p], NSKPX[p], NBGY[p], NEDY[p],
                                            NSKPY[p], NBGZ[p], NEDZ_EP,
                                            NSKPZ[p], coord);
                                        printf(
                                            "%d = (%d,%d)) "
                                            "NX,NY,NZ=%d,%d,%d\nnxt,nyt,nzt=%d,"
                                            ...
                                            NX * grdfct[p], NY * grdfct[p],
                                            nzt[p], nxt[p], nyt[p], nzt[p],
                                            rec_NX[p], rec_NY[p], rec_NZ[p],
                                            rec_nxt[p], rec_nyt[p], rec_nzt[p],
                                            NBGX[p], NSKPX[2], NEDX[2], NBGY[2],
                                            NSKPY[2], NEDY[2], NBGZ[2],
                                            NSKPZ[2], NEDZ_EP, rec_nbgx[p],
                                            rec_nedx[p], rec_nbgy[p],
                                            rec_nedy[p], rec_nbgz[p],
                                            rec_nedz[p],
                                            (long int)displacement[p]);

                                        /*this should save the final plastic
                                         * strain down to NEDZ_EP grip points*/
                                        Bufeta2[p] =
                                            Alloc1D(rec_nxt[p] * rec_nyt[p] *
                                                    rec_nzt[p]);

                                        for (k = nzt[p] + align - 1 -
                                                 rec_nbgz[p];
                                             k >=
                                             nzt[p] + align - 1 - rec_nedz[p];
                                             k = k - NSKPZ[p])
                                                for (j = 2 + ngsl + rec_nbgy[p];
                                                     j <=
                                                     2 + ngsl + rec_nedy[p];
                                                     j = j + NSKPY[p])
                                                        for (i = 2 + ngsl +
                                        ...
                                        err = MPI_Type_contiguous(
                                            rec_nxt[p], MPI_FLOAT, &filetype2);
                                        err = MPI_Type_commit(&filetype2);
                                        for (i = 0; i < rec_nyt[p]; i++) {
                                                dispArray2[i] = sizeof(float);
                                                dispArray2[i] = dispArray2[i] *
                                                                rec_NX[p] * i;
                                        }
                                        err = MPI_Type_create_hindexed(
                                            rec_nyt[p], ones2, dispArray2,
                                            filetype2, &filetype2);
                                        err = MPI_Type_commit(&filetype2);
                                        for (i = 0; i < rec_nzt[p]; i++) {
                                                dispArray2[i] = sizeof(float);
                                        ...
                                        printf(
                                            "filetype size "
                                            "(supposedly=rec_nxt*rec_nyt*rec_"
                                            "nzt*4=%ld) =%d\n",
                                            rec_nxt[p] * rec_nyt[p] *
                                                rec_nzt[p] * sizeof(float),
                                            tmpSize);

                                        sprintf(filename, "Finaleta_%d_%07ld",
                                                p, cur_step);
                                        err = MPI_File_open(
                                            MCW, filename,
                                        ...
                                        err = MPI_File_set_view(
                                            fh, displacement[p], MPI_FLOAT,
                                            filetype2, "native", MPI_INFO_NULL);
                                        if (err != MPI_SUCCESS) {
                                                fprintf(
                                                    stderr,
                                                    "MPI error in "
